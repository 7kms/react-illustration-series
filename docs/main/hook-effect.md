...
function commitLifeCycles(
  ...
  switch (finishedWork.tag) {
    ...
    case SimpleMemoComponent:
    case Block: {
      // 在此之前 commitMutationEffects 函数中，effect.destroy 已经被调用，所以 effect.destroy 永远不会影响到 effect.create
...
export function flushPassiveEffects(): boolean {
  ...
  if (pendingPassiveEffectsRenderPriority !== NoSchedulerPriority) {
    ...
    // `runWithPriority`设置 Schedule 中的调度优先级，如果在 flushPassiveEffectsImpl 中处理 effect 时又发起了新的更新，那么新的 update.lane 将会受到这个 priorityLevel 影响。
  ...
  if (currentHook !== null) {
    ...
    if (nextDeps !== null) {
      ...
      if (areHookInputsEqual(nextDeps, prevDeps)) {
        // 2.1 如果依赖不变，新建 effect(tag 不含 HookHasEffect)
