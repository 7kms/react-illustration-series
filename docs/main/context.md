...
export function readContext<T>(
  ...
  // 1. 构造一个 contextItem, 加入到 workInProgress.dependencies 链表之后
...
function updateContextProvider(
  ...
  if (oldProps !== null) {
    ...
    if (changedBits === 0) {
      ...
      // value 变动，查找对应的 consumers, 并使其能够被更新
      ...
      while (dependency !== null) {
        ...
        if (
          ...
          if (fiber.tag === ClassComponent) {
            // class 组件需要创建一个 update 对象，添加到 updateQueue 队列
            const update = createUpdate(
              NoTimestamp,
              pickArbitraryLane(renderLanes),
            );
            update.tag = ForceUpdate; // 注意 ForceUpdate, 保证 class 组件一定执行 render
