...
2. 中转: `react-reconciler`收到`更新需求`之后, 并不会立即构造`fiber树`, 而是去调度中心`scheduler`注册一个新任务`task`.
3. 调度: 调度中心`scheduler`通过`任务调度循环`来执行`task`.
4. 输出: `task`的执行过程又回到了`react-reconciler`包中.
