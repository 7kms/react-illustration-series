...
对于树或图结构的搜索(或遍历), 分为深度优先(DFS)和广度优先(BFS).
...
在`react-reconciler`包中, `ReactElement`的构造过程实际上也是`reconciler`调和过程. 与`fiber`树的构造是相互交替, 逐级进行的(在`fiber 树构建`章节中详细解读, 本节只介绍深度优先遍历的使用场景).
